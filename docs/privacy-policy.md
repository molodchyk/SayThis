...
- Wikidata and Wiktionary lookup; lookup language hints may be sent as Wikidata API language parameters and may choose matching Wiktionary language sections or target-language Wiktionary editions before English fallback
...
- a user-configured community endpoint, only after community submission, shared-audio use, or approved-entry refresh is enabled
...
A configured community service may host reviewed or service-generated audio artifacts for shared reuse. These artifacts are tied to term-level pronunciation metadata and do not include page URLs, browsing history, full-page text, or user recordings.

If a community service operator uses moderator/admin or authenticated provider generation, the service sends only the source form or term text, locale, optional voice name, and speaking rate to the configured speech provider. Hosted public shared-audio lookup reuses approved audio and does not synthesize new paid audio.
...
- disable shared-audio endpoint use for Speak actions
...
- disable community submission, shared-audio endpoint use, approved-entry refresh, custom source lookup, DBpedia-compatible lookup, Forvo lookup, and place-name lookup
