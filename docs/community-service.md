...
The shared-audio action first returns an already approved audio entry for the same lookup key and compatible language. It can also reuse an approved entry with the same term, source form, alias, or variant when both the request and the entry have the same base language. The extension sends bounded aliases and variants from the resolved result so approved shared audio can be reused across known written forms. The client-side shared-audio setting is separate from correction sync and approved-entry refresh, so Speak can reuse or request endpoint audio without submitting corrections. On hosted deployments, if no approved audio exists, the public shared-audio action returns `shared-audio-not-found` and does not call the provider. `SAYTHIS_PUBLIC_AUDIO_GENERATION_ENABLED=1` is a loopback-only development escape hatch: it still requires an `http://127.0.0.1` or `http://localhost` public base URL and a loopback client address before provider synthesis can run. Local development generation also requires a useful target, rejects plain same-text English requests, rejects non-English resolved languages routed to English TTS locales, and consumes a persisted local generation budget. Production provider generation belongs behind token-protected moderator endpoints or a future authenticated entitlement path. Service-generated provider output is published as generated shared audio with a `service-generated` trust signal; moderator-created artifacts use `moderator-reviewed`. The extension never stores provider credentials or generation tokens; provider auth, billing protection, and any hosted entitlement or paywall stay inside the community service. Reused approved audio remains public through approved-entry refresh and `/audio/<artifact-id>`.
...
- `SAYTHIS_PUBLIC_AUDIO_GENERATION_ENABLED`: `0` by default; set to `1` only for loopback local development generation. Hosted HTTPS deployments still cannot generate from public shared-audio requests.
- `SAYTHIS_PUBLIC_AUDIO_GENERATION_LIMIT`: `50`; local development generation budget. Set to `0` to exhaust local generation while keeping approved audio reuse available.
- `SAYTHIS_PUBLIC_AUDIO_GENERATION_WINDOW_MS`: `86400000`; local development generation budget window.
