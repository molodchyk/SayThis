...
| `credentials` | Remote source settings | Object normalized by `normalizeCredentials`; currently stores the optional Forvo API key and shared-audio generation token. | Kept until the user clears or changes credentials through options or import. | Local-only secret material for optional source access. | Never export API keys or generation tokens; keep credentials separate from ordinary settings. |
...
| `settings` | Extension settings | Object normalized by `normalizeSettings`; includes toggles, endpoints, labels, lookup hints, and sync flags. | Kept until changed in options, import, or storage reset. | User preferences and optional endpoint URLs. | Normalize before reads; endpoint permission cleanup must compare previous and next normalized settings. |
...
- Keep optional credentials in `credentials`, not in exports or sync queues.
