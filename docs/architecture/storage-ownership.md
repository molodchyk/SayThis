...
| `credentials` | Remote source settings | Object normalized by `normalizeCredentials`; currently stores optional API keys. | Kept until the user clears or changes credentials through options or import. | Local-only secret material for optional source access. | Never export API keys; keep credentials separate from ordinary settings. |
