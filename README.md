...
- Advanced diagnostics can probe the offscreen Web Speech page for voice availability, selected voice, matching voices, and offscreen speech errors.
...
- Voice-provider requirements are captured in `docs/product/voice-provider-requirements.md`: generated fallback audio should use high-quality provider voices, be cost-controlled, and become reusable shared audio after approval.
...
- Keep shared provider generation behind service-side opt-in, hosted entitlement or paywall controls, rate limits, useful-target validation, and persisted generation budgets.
